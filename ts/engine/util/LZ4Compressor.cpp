...
PosType LZ4Compressor::compressFullStream(const char *TS_RESTRICT src, SizeType srcSize, char *TS_RESTRICT dst, SizeType dstSize)
{
	...
	while (true)
	{
		...
		const int32_t compressedBytes = LZ4_compress_fast_continue(lz4Stream, srcPtr, dstPtr, (int32_t)srcBytesRemaining, (int32_t)dstBytesRemaining, 1);
...
PosType LZ4Compressor::decompressFullStream(const char *TS_RESTRICT srcPtrStart, SizeType srcSize, char *TS_RESTRICT dstPtrStart, SizeType dstSize)
{
	...
	while (true)
	{
		...
		const int32_t decompressedBytes = LZ4_decompress_safe_continue(lz4Stream, srcPtr, dstPtr, (int32_t)blockCompressedSize, (int32_t)dstBytesRemaining);
