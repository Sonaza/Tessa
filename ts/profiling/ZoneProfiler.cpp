...
public:
	...
	{
		const engine::window::WindowView &view = windowManager->getInterfaceView();
...
private:
	...
	{
		engine::system::BaseApplication &app = TS_GET_GIGATON().getGigaton<engine::system::BaseApplication>();
		debugFont = &app.getDebugFont();

		windowManager = &TS_GET_GIGATON().getGigaton<engine::window::WindowManager>();
...
void ZoneProfiler::render(sf::RenderTarget &renderTarget, const engine::window::WindowView &view)
{
#if TS_PROFILER_ENABLED == TS_TRUE
